...
class Parser():
    ...
    def iteration(self, var_name, i, pos, sp, k, end):
        code = ""
        self.symbol_table.define(var_name, i)
        while True:
            if self.current_token.type == END_FOR:
    ...
    def threaded_for_statement(self, var_name, sp, threads):
        ...
        if threads is None:
            with ThreadPool(processes=4) as pool:
                pool.starmap(self.iteration, [(var_name, i, pos, sp, k, end) for i in sp[1:]])
        else:
            with multiprocessing.Pool(processes=threads) as pool:
                pool.map(self.iteration, [(var_name, i, pos, sp, k, end) for i in sp[1:]])
